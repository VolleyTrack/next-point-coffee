...
export default async function AthleteDashboardPage() {
  ...
  const [campaigns, sales] = await Promise.all([
    listAllCampaigns(),
    listSales({ athleteId: user.athleteId }),
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-12 px-6 py-14">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">Only your campaigns and the sales attributed to you.</p>
      </div>

      <section className="grid grid-cols-2 gap-4 md:grid-cols-3">
        {[
          { label: "Your campaigns", value: String(mine.length) },
          { label: "Bags on your link", value: String(bags) },
      ...
      <section>
        <h2 className="text-2xl font-black text-np-cream">Sales on your name</h2>
          ...
          <table className="w-full min-w-[560px] text-left text-sm">
            <thead className="bg-card text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3">Owed to club</th>
              </tr>
            </thead>
            <tbody>
              {sales.map((sale) => (
                <tr key={sale.id} className="border-t border-gold/10">
                  ...
                  <td className="px-4 py-3 text-np-cream">{sale.buyerName}</td>
                  <td className="px-4 py-3 text-muted-foreground">
              ...
              {sales.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-6 text-center text-muted-foreground">
                    No sales attributed to you yet. Share your live link.
                  </td>
                </tr>
              )}
            </tbody>
