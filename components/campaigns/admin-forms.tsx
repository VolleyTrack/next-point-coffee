...
import { PARTNER_LOGIN_URL } from "@/lib/campaigns/login-email";
...
export function CreateSetupForm({ initialRequest }: { initialRequest?: CampaignRequest | null }) {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setEmailWarning(typeof data.emailWarning === "string" ? data.emailWarning : "");
      setOk(
        publish && slug
  ...
  return (
    <form onSubmit={submit} className="rounded-lg border border-gold/20 bg-card p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        ...
        Temporary passwords are shown once below and emailed to those addresses. Each person chooses their own
        password on first sign-in. Bag share is set on the organization — type does not lock the amount.
      </p>
      {initialRequest && (
      ...
      {emailWarning && (
        <p className="mt-3 rounded-md border border-red-400/40 bg-red-400/10 px-3 py-2 text-sm text-red-300" role="alert">
          {emailWarning}
        </p>
      )}
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                {emailWarning
                  ? "Email did not go out. Copy these and send them to the club and the athlete. They will not appear here again."
                  : "Emailed to the club and athlete, and shown once here. They will not appear here again."}{" "}
                Passwords are stored only as hashes. First sign-in requires a new password. Sign in at {PARTNER_LOGIN_URL}.
              </p>
            </div>
