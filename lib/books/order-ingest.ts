...
import { grindLabel, isGrindId, orderLineMaxQuantity, type GrindId } from "@/lib/site";
...
function booksItemsFromLineItems(lineItems: unknown): BooksOrderItem[] | undefined {
  ...
  for (const item of lineItems) {
    ...
    items.push({
      ...
      grind: row.grind,
      form,
      quantity: Number.isFinite(quantity) && quantity >= 1 ? Math.min(orderLineMaxQuantity, Math.floor(quantity)) : 1,
    });
  }
