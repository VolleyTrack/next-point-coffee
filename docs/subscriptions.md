...
## Checkout
Inline `price_data` (no Stripe products/prices to create): name like `First Serve Subscription — Whole bean`, `unit_amount` = subscriber per-bag cents, `recurring { interval: "week", interval_count: 2 | 4 | 6 }`, `quantity` = bags (no bag limit; 1-999 sanity ceiling), with `adjustable_quantity` on (min 1, max 999) so the count can change on the Stripe page. Renewals use the billed invoice line quantity, not `sub_bags`, so a Checkout or portal change is honored. Books ingest accepts up to 999 per line (the one-time shop stays at 99). US shipping address collection. Promotion codes off. Metadata on the session (per delivery) and on `subscription_data.metadata` (plan): `order_type=subscription`, `sub_coffee`, `sub_grind`, `sub_bags`, `sub_frequency`, plus the retail keys (`productSlug`, `grind`, `form`, `quantity`, `items`).
